...
> **Overview:** Homewolves uses a glassmorphism + bento grid aesthetic inspired by JamesEdition. The design communicates trust, luxury, and clarity for the African real estate market. All tokens are defined as CSS custom properties in DESIGN.md. This file is a quick-reference summary — agents building UI must read DESIGN.md in full before writing frontend code.

---

## Visual Language

> **Section summary:** Monochrome neutral base with warm amber accent. Two typefaces: DM Serif Display (headings/prices) and Inter (UI/body). 4px spacing scale.
...
**Dark theme:** All surface tokens inverted. Ambers stay warm. See DESIGN.md §2.1.

**High-contrast theme:** Black/white with `#FFD700` accent. See DESIGN.md §2.1.
...
> **Section summary:** All interactive primitives come from shadcn/ui, wrapped in `Hw*` branded components. Feature code never imports shadcn directly — only from `@/components/ui`. See DESIGN.md §3.0 for the full wrapper pattern.
...
### Forms
- Input fields: White bg, `--radius-md` (12px), `--shadow-xs`, focus ring `--color-brand-secondary`
...
- **Dashboard (tablet/desktop):** Side nav (64px collapsed / 240px expanded), glass surface
...
### Modals / Dialogs
- Entry: Scale from 0.96 + fade, `--duration-normal --ease-spring`
...
> **Section summary:** Six core UX rules that govern all interaction design on the platform.
...
> **Section summary:** Mobile-first. All layouts start at 375px and expand through 7 defined breakpoints.
...
> **Section summary:** Minimum accessibility standards from DESIGN.md §10. All must pass before a component is considered complete.
